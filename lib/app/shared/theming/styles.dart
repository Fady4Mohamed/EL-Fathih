import 'package:el_fathih/app/shared/theming/color.dart';
import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';

class TextStyles {
  static TextStyle font10GreenRegular = TextStyle(
    color: Colors.green,
    fontSize: 10.sp,
    fontWeight: FontWeight.w400,
  );
  static TextStyle font10RedRegular = TextStyle(
    color: Colors.red,
    fontSize: 10.sp,
    fontWeight: FontWeight.w400,
  );
  static TextStyle font30whiteExtraBold = TextStyle(
    color: Colors.white,
    fontSize: 30.sp,
    fontWeight: FontWeight.w700,
    height: 1,
  );
  static TextStyle font35whiteExtraBold = TextStyle(
    color: Colors.white,
    fontSize: 35.sp,
    fontWeight: FontWeight.w700,
    height: 1.2,
  );
  static TextStyle font7white700 = TextStyle(
    color: Colors.white,
    fontSize: 7.sp,
    fontWeight: FontWeight.w700,
  );
  static TextStyle font17whiteSemiBold = TextStyle(
    color: Colors.white,
    fontSize: 17.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle font20whiteSemiBold = TextStyle(
    color: Colors.white,
    fontSize: 20.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle font20BlackSemiBold = TextStyle(
    color: Colors.black,
    fontSize: 20.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle font18GrayRegular = TextStyle(
    color: ColorsManager.normalGray,
    fontSize: 18.sp,
    fontWeight: FontWeight.w400,
  );
  static TextStyle font20NevySemiBold = TextStyle(
    color: ColorsManager.main,
    fontSize: 20.sp,
    fontWeight: FontWeight.w500,
  );
static TextStyle font14NevySemiBold = TextStyle(
    color: ColorsManager.main,
    fontSize: 14.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle font16NevySemiBold = TextStyle(
    color: ColorsManager.main,
    fontSize: 16.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle font15whiteRegular = TextStyle(
    color: Colors.white,
    fontSize: 15.sp,
    fontWeight: FontWeight.w400,
  );
  static TextStyle font16grayRegular = TextStyle(
      color: Colors.white.withValues(alpha: 0.6),

    fontSize: 16.sp,
    fontWeight: FontWeight.w400,
  
  );
  static TextStyle font14GrayRegular = TextStyle(
    color: ColorsManager.lightGray,
    fontSize: 14.sp,
    fontWeight: FontWeight.w400,
  );
  static TextStyle font13whiteRegular = TextStyle(
    color: Colors.white,
    fontSize: 13.sp,
    fontWeight: FontWeight.w400,
  );
}
